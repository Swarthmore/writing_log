...
<?php
if ($node->solutions){
	if ($start_date == $end_date AND $start_date > time()){
		print t('- provided on ') . format_date($start_date, 'short');
	} else if ($start_date != $end_date AND $end_date > time() AND $start_date < time()){
		print t('- provided  until ') . format_date($end_date, 'short') ;
